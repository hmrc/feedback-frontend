...
class ComplaintFeedbackQuestionsViewSpec
    extends YesNoViewBehaviours[ComplaintFeedbackQuestions]
    with StringViewBehaviours[ComplaintFeedbackQuestions]
  ...
  def createView: () => HtmlFormat.Appendable = () =>
    complaintFeedbackQuestionsView(frontendAppConfig, form, action)(fakeRequest, messages)
  ...
  "ComplaintFeedbackQuestions view" must {
    ...
    behave like stringPage(createViewUsingForm, "whyGiveScore", "complaintFeedbackQuestions.whyGiveScore")
    ...
    "contain second introductory paragraph" in {
      val expectedMessage =
        messages("complaintFeedbackQuestions.intro2", messages("complaintFeedbackQuestions.introLinkText"))
