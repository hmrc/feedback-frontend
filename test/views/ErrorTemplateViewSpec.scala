...
class ErrorTemplateViewSpec extends ViewBehaviours {
  ...
  "ErrorTemplate view" must {
    "render the correct title" in {
      val expectedMessage = "title"

      val doc = asDocument(createView())
      assert(doc.title().equals(expectedMessage))
    }
    "contain correct heading" in {
      val expectedMessage = "heading"

      val doc = asDocument(createView())
      assertPageTitleEqualsMessage(doc, expectedMessage)
    }
    "contain relevant error information" in {
