...
class TrustsQuestionsViewAccessibilitySpec
  extends AnyWordSpec
    with Matchers
  ...
  val request: FakeRequest[AnyContentAsEmpty.type] = FakeRequest("GET", "/hmrc-frontend/trusts")
  val messages: Messages = app.injector.instanceOf[MessagesApi].preferred(request)

  val pageTemplate: TrustsQuestionsView = app.injector.instanceOf[TrustsQuestionsView]

  "The Trusts Questions View page" must {
