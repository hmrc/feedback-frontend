...
class NmwCcgQuestionsViewAccessibilitySpec
  extends AnyWordSpec
    with Matchers
  ...
  val request: FakeRequest[AnyContentAsEmpty.type] = FakeRequest("GET", "/hmrc-frontend/:origin/nmw")
  val messages: Messages = app.injector.instanceOf[MessagesApi].preferred(request)

  val pageTemplate: NmwCcgQuestionsView = app.injector.instanceOf[NmwCcgQuestionsView]

  "The NMW CCG Questions View page" must {
